### Code I built up step by step watching his video: https://www.youtube.com/watch?v=l8pRSuU81PU
# See other file for original code
...
class CausalSelfAttention(nn.Module):
    ...
    #     # eg in GPT-2 (124M), n_heads=12, head_size=64, so nh*hs=768 channels in the Transformer
    #     qkv = self.c_attn(x) # create a linear projection to get the q,k,v vectors
...
train_loader = DataLoaderFineWebEdu(B=B, T=T, process_rank=ddp_rank, num_processes=ddp_world_size, split='train')
val_loader = DataLoaderFineWebEdu(B=B, T=T, process_rank=ddp_rank, num_processes=ddp_world_size, split="val")
...
min_lr = max_lr * 0.1
warmup_steps = 715
max_steps = 19073 * 7# 19073 -> 1 epoch for 10B tokens and batch size 0.5M tokens
def get_lr(it):
...
for step in range(max_steps):
    ...
    # # do gradient accumulation instead
    ...
    for micro_step in range(grad_accum_steps):
        ...
        x, y = x.to(device), y.to(device)
        # with torch.autocast(device_type=device, dtype=torch.bfloat16): # for A100 & beyond
        with torch.autocast(device_type=device_type, dtype=torch.bfloat16):
            logits, loss = model(x, y)
